...
\]
and there is equal chance of entering each scenario. We then have the expected cost difference per share, given the same block trade size and other factors, between executing in the call auction and during the continuous phase, is:
\begin{equation}\label{eqn:expected_auction_saving}
  E[\zeta] =  \frac{e^{V_0}}{2 b_e}
\end{equation}
which shows that even with the same pricing functions, the cost of trading during the auction is smaller than during the continuous phase. Heuristically speaking, trading in the call auction offers the liquidity trader a chance equivalent to immediately getting filled into the spread during the continuous trading phase. This allows us to derive the expected markup price that the liquidity trader receives for trading during the opening auction is
\begin{equation}\label{eqn:exp_markup_px_eqb}
  p_{avg} = p_0 + \frac{2 - e^{V_0}}{b_e} x_0
\end{equation}
...
\[
  \hat{\alpha} = \frac{2 - e^{\bar{V_e}}}{\hat{b_e}}
\]
...
\begin{proof}[Derivation of the optimal trading strategy]\label{proof:optimal-strategy}
  ...
  \]
  \[
    \Leftrightarrow \frac{d}{d\epsilon} \left[\int_0^T (B_0 \alpha e^{-\rho s} \hat{E_s} + \beta \hat{E_s}^2 ) ds  + \int_0^T (B_0 \alpha e^{-\rho s}\epsilon {v'}_s + 2\beta\hat{E_s}\epsilon{v'}_s + \beta\epsilon^2{v'}^2_s)ds\right]|_{\epsilon=0}=0
  \]
  \[
