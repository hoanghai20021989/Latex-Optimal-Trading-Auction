...
where
\[
  F_1 = 4 e^{T \rho} \hat{\beta} \rho [-\alpha + e^{T \rho} (\alpha + 2 \hat{\beta} \rho)]
\]
\[
  F_2 = \alpha^2 (2 + T \rho) - 4 e^{T \rho} \alpha (\alpha + 2 \hat{\beta} \rho)
  + e^{2 T \rho} [8 \hat{\beta} \alpha \rho + 8 \hat{\beta} (T \alpha + \hat{\beta}) \rho^2 + \alpha^2 (2 - T \rho)]
\]
The $\hat{\beta}$ is an optimized value of $\beta$ that satistfies the monotonic constraints using the maximin algorithm in the proof. As a result, the optimal block trade $B_0$ during auction is a fraction of the total position $Q_0$ that depends on the ratio of the cost impact factors. The optimal trading strategy during continuous session is:
\[
  \hat{X_t} = Q_0 - B_0  - \frac{B_0 \alpha}{2 \hat{\beta} \rho}(1 - e^{-t \rho}) - \frac{e^{-T \rho} (B_0 \alpha - e^{T \rho}B_0 \alpha + 2 e^{T \rho} (Q_0 - B_0) \hat{\beta} \rho )}{2 T \hat{\beta} \rho} t
\]
...
\[
  C = \alpha B_0^2 + \frac{(e^{-2 T \rho} - 1) T \alpha^2 \rho B_0^2 + 2 [2 Q_0 \hat{\beta} \rho + \{(e^{-T \rho} - 1) \alpha - 2 \hat{\beta} \rho\} B_0]^2}{8 T \hat{\beta} \rho^2}
\]
...
\begin{proof}[Derivation of the optimal trading strategy]\label{proof:optimal-strategy}
  ...
  \[
    C = \alpha B_0^2  + \alpha \int_0^T (B_0 + V_e) e^{-\rho s} E_s ds + \beta \int_0^T E_s^2 ds
  \]
  ...
  \[
    \min_{(X_t, E_t)} \int_0^T (B_0 + V_e) \alpha e^{-\rho s} E_s ds + \int_0^T \beta E_s^2 ds
  \]
  subject to
  \[
    E_t > 0,\ \forall t
  ...
  \[
    \int_0^T [ B_0 \alpha e^{-\rho s} E_s + (\beta + \lambda I_{E_s \leq 0}) E_s^2 ] ds=\int_0^T L(s, X_s, E_s) ds
  \]
  where $\lambda$ is the Lagrange multiplier and $L(s, X_s, E_s)$ is the Lagrangian of the functional. Let
  \[
    \hat{\beta} =  \frac{\int_0^T (\beta + \lambda I_{E_s \leq 0}) E_s^2 ds}{\int_0^T E_s^2 ds}
  \]
  we then can solve for the following maximin optimization problem
  \[
    \max_{\hat{\beta}} \min_{E_t} \left[ \alpha B_0^2  + \alpha \int_0^T B_0 e^{-\rho s} E_s ds + \hat{\beta} \int_0^T E_s^2 ds \right]
  \]

  Consequently, we optimize for the following functional to find the optimal $E_t$ for each $\hat{beta}$
  \[
    \int_0^T [ B_0 \alpha e^{-\rho s} E_s +  \hat{\beta} E_s^2 ] ds=\int_0^T L(s, X_s, E_s) ds
  \]
  First we assume $B_0$ is static, and solve the cost of trading during continuous phase:
  \[
    C_c = \int_0^T B_0 \alpha e^{-\rho s} E_s ds + \int_0^T \beta E_s^2 ds
  \]
  which has the following Euler-Lagrange equation
  \[
    \frac{\alpha}{2 \hat{\beta}} B_0  e^{-\rho s} +E_s = A
  \]
  \[
    \Leftrightarrow \hat{E_s} = A - \frac{B_0 \alpha}{2\hat{\beta}} e^{-\rho s}
  \]
  \[
    \Leftrightarrow \hat{X_t} - \hat{X_0} = At + B - \frac{B_0 \alpha}{2\hat{\beta} \rho} (1 - e^{-\rho t})
  \]
  ...
  \]
  \[
    A = \frac{2 (B_0 - Q_0 + \lambda T) + \frac{(1 - e^{-T \rho})}{\hat{\beta} \rho} B_0 \alpha} {2 T}
  \]
  \[
    \Leftrightarrow \hat{X_t} = Q_0 - B_0  - \frac{B_0 \alpha}{2 \hat{\beta} \rho}(1 - e^{-t \rho}) - \frac{e^{-T \rho} (B_0 \alpha - e^{T \rho}B_0 \alpha + 2 e^{T \rho} (Q_0 - B_0) \hat{\beta} \rho )}{2 T \hat{\beta} \rho} t
  \]
  ...
  \[
    ...
    \end{split}
  \]
  Given the cost of the optimal strategy for each  $\hat{\beta}$, we then can find the maximum value of $\hat{\beta}$ using any derivative-free root finding algorithm, such as the Nelder-Mead algorithm (\cite{NelderMead1965}).
